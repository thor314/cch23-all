...
pub struct ElapsedState {
  pub elapsed_map: HashMap<String, Instant>,
}

pub async fn store_string(State(elapsed_state): State<SharedState>, Path(s): Path<String>) {
...
/// ]
/// ```
pub async fn ulids_to_uuids(Json(ulids): Json<Vec<String>>) -> Json<Vec<Uuid>> {
